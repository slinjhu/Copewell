function mapdraw(container, datafile, colname, min, max){
    var datamap;
    var colorMap = new Choropleth(container);

    queue()
    .defer(d3.json, 'us-counties.json')
    .defer(d3.csv, datafile)
    .await(function(err, json, csv){
        var countydata = new CountyData(csv, colname, min, max);
        colorMap.loadMap(json, countydata);
        colorMap.addLegend(countydata);
        d3.select(container)
...
var CountyData = function(csvdata, colname, min, max){
    datamap = d3.map();
    csvdata.forEach(function(d){
        datamap.set(d.fips, +d[colname]);
    });
